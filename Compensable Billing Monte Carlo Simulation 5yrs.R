...
results2 = NULL
y = 5 # Number of years to simulate
...
for (k in 1:50000) # Number of iterations
{
  ...
  an.comp <- 3287
      ...
      results = NULL
      for (k in 1:y) # Simulates y years of data
        {
        ...
        cost <- an.comp*cost.patient*30
        total <- price.act - cost
        
        results = rbind(results, data.frame(total))
        }
    
    npv <- 69756.69 + NPV(cf0=0, cf=results$total, times=1:y, i=0.03, plot=FALSE)
    # npv <- 180750.00 + NPV(cf0=0, cf=results$total, times=1:y, i=0.03, plot=FALSE)
